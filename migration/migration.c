...
static QemuMutex ft_sync_mutex;
...
{
    MigrationState *s = opaque; //cocotion fucking crazy
...
/*
		...
		FILE *pFile;
		char pbuf[10];
    	pFile = fopen("mytestdata.txt", "rw");
		if(fgets(pbuf, 10, pFile) != NULL) {
		int num = atoi(pbuf);
		printf("cocotion fucking pid = %d\n", pid)	;
		printf("cocotion fucking num = %d\n", num)	;

		if(pid < num) {
			//okokokok
			global_enable_trans_wait = true;
		} else {
			sprintf(pbuf, "%d", pid);
			fputs(pbuf, pFile);
		}
 ...
 //           printf("fucking you serious??? index = %d\n", migrate_get_index(s));
  //          getchar();
    //    }
        ...
        /*printf("cocotion test now state index= %d, state = %d\n", migrate_get_index(s), s->ft_state);
        if(s->ft_state == CUJU_FT_TRANSACTION_PRE_RUN)
            printf("holy shit! I need return, cuz state is trans pre run\n");
        else if(migrate_token_owner == s && s->ft_state != CUJU_FT_TRANSACTION_PRE_RUN)
            //printf("holy shit! I need return, cuz token is not own\n");
            printf("holy shit! I need return, cuz state is shit! but token is ok\n");
*/
...
//	s->snapshot_start_time = (double) cuju_sync_local_VMs_runstage(2) / 1000000;
	...
	//if(global_enable_trans_wait) sleep(1); //cocotion fucking test
