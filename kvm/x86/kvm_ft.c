...
static int bd_predic_stop3(void)
{
    ...
    //bd_prekvm_shm_vcpu_timer_callbackdic_stop2();
...
/*
    int newdirty = global_predict_dirty_rate * (epoch_run_time - epoch_time_old_global);
    int beta = (global_current_dirty_byte+newdirty)/global_last_trans_rate + epoch_run_time;

	printk("============\n");
	printk("cocotiion test epoch_run_time 1.  = %d, beta = %d\n", epoch_run_time, beta);
	printk("cocotiion test newdirty 1.  = %d\n", newdirty);
	printk("cocotion test guess all dirty 1. = %d\n", global_current_dirty_byte+newdirty);
	printk("cocotion test guess trans_rate 1. = %d\n", global_last_trans_rate);
	printk("============\n");
...
//    update->predic_trans_rate = D*t;
    ...
    //if(global_total_last_transfer_bytes != 0) {
		...
		//unsigned long int predic_t_bytes = global_total_last_transfer_bytes;


        //printk("predic_t_bytes now is %d\n", predic_t_bytes);
        //if (global_predic_t_bytes == 0) global_predic_t_bytes = global_total_last_transfer_bytes;

        //predic_t_bytes = predic_t_bytes * ((global_total_last_transfer_bytes*1000)/global_predic_t_bytes);
        //predic_t_bytes /= 1000;
        //printk("predic_t_bytes next is %d\n", predic_t_bytes);
        //int factor = global_predic_t_bytes*1000/global_total_last_transfer_bytes;
        //int factor = global_total_last_transfer_bytes*1000/global_predic_t_bytes;


        //global_predic_t_bytes = predic_t_bytes;
		//int predic_t = predic_t_bytes/R;
		//t = target_latency_us - 500 - (predic_t + E);

       // t = (global_total_last_transfer_bytes - x) / D;

        //printk("cocotion test predic_t = %d, E = %d\n", predic_t, E);
	//}
    //if(t < 10)  t = 100;
...
//        printk("cocotion test runtime at fucking timer interrupt @@@@@@@@ = %d\n", runtime_difftime);
        ...
        //printk("cocotion test fucking update 2 fuck@!!! to take snapshot in kvm_shm_vcpu_timer_callback\n");
...
/*
	if(global_current_100_count < 1000)
		...
		if(global_victim >= 1000) global_victim = 0;
...
static unsigned long __gfn_to_hva_many_new(struct kvm_memory_slot *slot, gfn_t gfn,
			   ...
			   bool *writable, pfn_t *pfn)
{
    ...
    struct task_struct *current_backup;

 //   spin_lock(&myftlock);
    //struct task_struct *current_backup = current_thread_info()->task;
    //current_thread_info()->task = myglobaltask;

   // if(current != myglobaltask)
    //    printk("cocotion test current defer to myglobaltask @@@@@@@@ \n");



   // struct task_struct *current_backup = curr_arc;
   // curr_arc = myglobaltask;
...
//    preempt_disable();
  ...
  //  if(current != current_backup)
   //     printk("cocotion test yes not equ\n");
    //else
     //   printk("cocotion test yes equ\n");

  //  __this_cpu_write(current_task, myglobaltask);

    //get_cpu_ptr(current_task);
    //current_backup = current_task;
    //current_task   = myglobaltask;
//    if(current != current_backup) {
 //       printk("cocotion test good change\n");
  //  } else if (current != myglobaltask) {
  //      printk("cocotion test yes not equal others one\n");
   // }
    if(current_backup != myglobaltask) {
      __this_cpu_write(current_task, myglobaltask);

        //if (current == myglobaltask) {
        //    printk("cocotion test yes changed\n");
        //}
       // else if (current == current_backup) {
        //    printk("cocotion test nothing changed\n");
        //}
	  might_sleep();
	  if (writable)
...
//    if(current == myglobaltask)
 //       printk("cocotion test current already same as myglobaltask\n");
  //  else
   //     printk("cocotion test current defer to myglobaltask\n");
...
//    __this_cpu_write(current_task, current_backup);
    ...
    //current_task = current_backup;
    //put_cpu_ptr(current_task);

 //   preempt_enable();

    //current_thread_info()->task = current_backup;
    //curr_arc = current_backup;
...
//		*writable = write_fault;
    ...
    //Here async is NULL, so I comment out
//	if (async) {
//		down_read(&myglobaltask->mm->mmap_sem);
//		npages = get_user_page_nowait(myglobaltask, myglobaltask->mm,
//					      addr, write_fault, page);
//		up_read(&myglobaltask->mm->mmap_sem);
//	} else
//
//
//
...
//						   FOLL_TOUCH|FOLL_HWPOISON);
    ...
    //this_cpu_write(current_task, current_backup);
    //preempt_enable();
   // current_thread_info()->task = current_backup;

  //  spin_unlock(&myftlock);
//    } else {
 //       printk("cocotion test myftlock is NULL\n");
...
/*
pfn_t __gfn_to_pfn_memslot_new(struct kvm_memory_slot *slot, gfn_t gfn, bool atomic,
			   bool *async, bool write_fault, bool *writable)
{
	unsigned long addr = __gfn_to_hva_many_new(slot, gfn, NULL, write_fault);
...
pfn_t gfn_to_pfn_memslot_new(struct kvm_memory_slot *slot, gfn_t gfn)
{
	return __gfn_to_pfn_memslot_new(slot, gfn, false, NULL, true, NULL);
}


pfn_t gfn_to_pfn_new(struct kvm *kvm, gfn_t gfn)
{
	return gfn_to_pfn_memslot_new(gfn_to_memslot(kvm, gfn), gfn);
}

struct page *gfn_to_page_new(struct kvm *kvm, gfn_t gfn)
{
	pfn_t pfn;

	pfn = gfn_to_pfn_new(kvm, gfn);
...
//    for (i = count/3; i < count-count/3; ++i) {
 ...
 //
	...
	if(real_count != 0) //cocotion fucking
	    total_dirty_bytes = (total_dirty_bytes/real_count)*count; //cocotion fucking
