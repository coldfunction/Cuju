...
#define KNUM 200 //current ok
#define KNUM2 200 //current ok
...
//#define LEARNRATE 600
#define LEARNRATE 300
//#define LEARNRATE 100;
//#define LEARNRATE 50
//#define LEARNRATE 200;
//#define LEARNRATE 800;
//#define LEARNRATE 400;
...
spinlock_t transfer_lock[16];
spinlock_t flush_lock[16];
...
struct ft_multi_trans_h {
	...
	int w4;
	int L3cache_speed;
	int L3cache_speed_rec[KNUM2];
	int L3cache_speed_c;
	int callsnapshot;
	int current_trans_rate;
	int compress_copy[KVM_DIRTY_BITMAP_INIT_COUNT];
	int compress_copy_c;
	int flush_c;
};
...
static int compare_dis(const void *lhs, const void *rhs)
{
	...
	struct k_dis lhs_d = *(const struct k_dis *)(lhs);
	struct k_dis rhs_d = *(const struct k_dis *)(rhs);
...
//	printk("cocotion test vcpu end start timer================== %p\n", vcpu);
    ...
    ctx = &kvm->ft_context;
	ft_m_trans.trans_time[ctx->cur_index][kvm->ft_id] = kvm->w3[0]/1000;
...
//	for(i = 0; i < try_t; i += 300) {
	for(i = 0; i < try_t; i ++) {
		...
		int tmp0 = dirty_bytes /  ft_m_trans.current_send_rate;
       	 ...
       	 int newbeta = tmp0*otherkvm->w0[0] + tmp1*otherkvm->w1[0] + otherkvm->w3[0] + IF*otherkvm->w4;
...
int when_take_snapshot(struct kvm *kvm, int beta, int epoch_run_time, int target_latency_us)
{
	...
	for(i = 0; i < try_t; i += 300) {
		...
		int tmp0 = dirty_bytes /  ft_m_trans.current_send_rate;
       	 ...
       	 int newbeta = tmp0*kvm->w0[0] + tmp1*kvm->w1[0] + kvm->w3[0];
		 newbeta/=1000;
		 if(newbeta + runtime >= target_latency_us) {
			 //printk("cocotion test ok when %d\n", i);
			return i;
	...
	for(i = 0; i < vm_counts; i++)	{
		if(i != kvm->ft_id) {
			...
			int tmp0 = kvm->old_dirty_count/ft_m_trans.current_send_rate;
       		...
       		beta += tmp0*kvm->w0[0] + tmp1*kvm->w1[0] + kvm->w3[0];
...
int get_l3s_start(struct kvm *kvm)
{
	//if(kvm->mflag == 0)
		native_write_msr(0x187,0x43412E,0); //L3 cache miss
	//kvm->mflag = 1;
	kvm->cache_start = native_read_msr(0xc2);
	kvm->cache_time_start = time_in_us();

//	uint64_t cache_miss3 = native_read_msr(0xc2); //L3 caches
//	int difftime = time_in_us() - kvm->cache_time_start;

//	if(difftime) {
//		return (cache_miss3-kvm->cache_start)*1000/difftime;
//	} else {
//		return 0;
//	}
	return 0;
...
int get_l3s_end(struct kvm *kvm)
{
	uint64_t cache_miss3 = native_read_msr(0xc2); //L3 caches
	kvm->cache_end = cache_miss3;
	int difftime = time_in_us() - kvm->cache_time_start;
	kvm->part_miss_time_rec = difftime;
	...
	if(difftime) {
//		return (cache_miss3-kvm->cache_start)*1000/difftime;
		int k = (cache_miss3-kvm->cache_start)*1000/difftime;
		//if(k < 0) {
			//if(kvm->ft_id == 0)
			//printk("miss_end = %ld, miss_start = %ld,D %ld time = %d speed = %d\n", cache_miss3, kvm->cache_start, cache_miss3-kvm->cache_start, difftime, k);
		//int m = kvm->load_mem_rate_rec_index[ctx->cur_index];
		/*if(kvm->trans_start) {
			int m = kvm->load_mem_rate_rec_index[kvm->trans_index];
//		kvm->load_mem_rate_rec[ctx->cur_index][m] = k;
			kvm->load_mem_rate_rec[kvm->trans_index][m] = k;
			kvm->load_mem_rate_rec_index[kvm->trans_index]++;
			if(kvm->load_mem_rate_rec_index[kvm->trans_index] >= 200)
				kvm->load_mem_rate_rec_index[kvm->trans_index] = 0;
		}*/
		kvm->miss_difftime = difftime;
		//}
		return (cache_miss3-kvm->cache_start)*1000/difftime;
	} else {
		return 0;
...
//		printk("kvmid = %d enter calc_miss3 yet, kick2 = %d\n", kvm->ft_id, kvm->ft_kick2);
		...
		spin_lock(&transfer_lock[kvm->ft_id]);
		if(kvm->trans_start) {
			kvm->trans_index = (kvm->trans_index+1) % 2;
			kvm->timestamp_index[kvm->trans_index] = 0;
			kvm->trans_start = 0;
		}
		spin_unlock(&transfer_lock[kvm->ft_id]);
		int k = kvm->trans_index;
		kvm->timestamp[k][kvm->timestamp_index[k]] = time_in_us();
		get_l3s_start(kvm);
		kvm->load_mem_rate_rec[k][kvm->timestamp_index[k]] = kvm->cache_start;
		//printk("%d %d %ld %ld\n", k, kvm->timestamp_index[k], kvm->timestamp[k][kvm->timestamp_index[k]], kvm->load_mem_rate_rec[k][kvm->timestamp_index[k]]);

		kvm->timestamp_index[k] = (kvm->timestamp_index[k]+1)%500;
...
int get_predict_L3cacheSpeed(struct kvm *kvm, int current_dirty_byte, int current_L3cache_speed)
{

	long long x = current_dirty_byte;
	long long y = current_L3cache_speed;
	int i;
	int sel = 8;
	int total = 0;
	if(kvm->latency_total > KNUM2) {
		for(i = 0; i < KNUM2; i++) {
			if(kvm->kpoint2[i].current_L3cache_speed) {
				kvm->kdis[i].index = i;
				long long xx = kvm->kpoint2[i].current_dirty_byte;
				long long yy = kvm->kpoint2[i].current_L3cache_speed;
				//kvm->kdis[i].value = (x-xx)*(x-xx)+(y-yy)*(y-yy);
				kvm->kdis[i].value = (y-yy)*(y-yy);
				total++;
			}
		}
...
/*
	...
	long long sum = 0;
	int knum = KNUM2;
	int N = (1+knum/2)*knum/2/2;
	for(i = 0; i < knum/2; i++) {
		int index = kvm->kdis[i].index;
		mk = kvm->kpoint2[index].actual_L3cache_speed;
		sum+=(knum/2-i)*mk;
	}
	mk = sum/N;
*/
	...
	//	return kvm->kpoint2[i].actual_L3cache_speed;
	return mk;
}
...
long long get_predict_trans(struct kvm *kvm, long long dirty_byte, long long transR) {
	long long x = dirty_byte;
	long long y = transR;
	long long mk = 1000;
	...
	if(kvm->latency_total > KNUM) {
		for(i = 0; i < KNUM; i++) {
			kvm->kdis3[i].index = i;
			long long xx = kvm->kpoint3[i].dirty_byte;
			//long long yy = kvm->kpoint3[i].trans_time;
			//kvm->kdis3[i].value = (x-xx)*(x-xx)+(y-yy)*(y-yy);
			kvm->kdis3[i].value = (x-xx);
			total++;
		}
		...
		for(i = 0; i < KNUM3; i++) {
			...
			mk = kvm->kpoint3[index].trans_time;
			//sum+=mk;
//			sum+=(mk*(max_dis-kvm->kdis[i].value))/max_dis;
			sum+=(KNUM3-i)*mk;
		}
//		mk = sum/50;
		mk = sum/N;
...
long long get_predict_trans_rate_s(struct kvm *kvm, long long dirty_pfns_len, long long dirty_len) {
	long long x = dirty_pfns_len;
...
long long get_predict_trans_rate_b(struct kvm *kvm, long long dirty_byte, long long transR) {
	long long x = dirty_byte;
	long long y = transR;
	long long mk = 0;
	...
	if(kvm->latency_total > KNUM4) {
		for(i = 0; i < KNUM4; i++) {
			...
			//long long yy = kvm->kpoint3[i].trans_time;
			//kvm->kdis3[i].value = (x-xx)*(x-xx)+(y-yy)*(y-yy);
			kvm->kdis[i].value = (x-xx);
			total++;
		}
		sort(kvm->kdis, total, sizeof(struct k_dis), &compare_dis, NULL);


		//int index = kvm->kdis3[0].index;
		//mk = kvm->kpoint3[index].trans_time;
		...
		for(i = 0; i < KNUM3; i++) {
			int index = kvm->kdis[i].index;
			mk = kvm->kpoint2[index].current_L3cache_speed;
			sum+=(KNUM3-i)*mk;
		}
		mk = sum/N;
	}
...
{
	//printk("%d %d\n", estimated_transtime, L3cache_speed);
	long long x = estimated_transtime;
	...
	if(kvm->latency_total > KNUM) {
		for(i = 0; i < KNUM; i++) {
			if(kvm->kpoint[i].L3cache_speed) {
				kvm->kdis[i].index = i;
				long long xx = kvm->kpoint[i].estimated_transtime;
				long long yy = kvm->kpoint[i].L3cache_speed;
				kvm->kdis[i].value = (x-xx)*(x-xx)+(y-yy)*(y-yy);
				total++;
			}
		}
	//printk(">>>>>>>>>>>>>>>>>>>> total = %d", total);
	kvm->err_total = total;
	sort(kvm->kdis, total, sizeof(struct k_dis), &compare_dis, NULL);
...
/*
	...
	for(i = 0; i < 50; i++) {
		...
		if(kvm->kdis[i].value > max_dis) {
			max_dis = kvm->kdis[i].value;
		}
	}
	...
	for(i = 0; i < 50; i++) {
			...
			if(mk > 100) mk = 100;
			//a[mk+100]++;
			a[mk+100] += (max_dis+1)/(kvm->kdis[i].value+1);
	}
...
//	}
	...
	int real_trans = (mk*estimated_transtime+100*estimated_transtime)/100;
...
*/
	int mk2 = real_trans-estimated_transtime;
	...
	for(i = 0; i < 50; i++ ) { //current best
	//for(i = 0; i < 500; i++ ) {
//	for(i = 0; i < 160; i++ ) {
		int index = kvm->kdis[i].index;
		long long dis = kvm->kdis[i].value;

		//printk("%d %ld\n", i, dis);

		int pre = kvm->kpoint[index].transtime_err;
//		dis/=10000;
		//a[pre]+=8000000/(dis+1);
		//printk("a = %ld\n", a[pre]);
//		a[pre]+=(double)1/dis;
...
static struct kvm_vcpu* bd_predic_stop2(struct kvm_vcpu *vcpu)
{
	...
	//diff_and_transfer_all(kvm, ctx->cur_index, 1);


    long long beta;
	long long beta2;
	int current_send_rate;
	long long current_dirty_byte = 0;
	...
	int guest_compress_dt = 0;
	int guest_load_r = 0;
	if(kvm->f_count) {
		guest_compress_dt = (dt*1000/kvm->f_count)*dlist->put_off;
		//printk("@@!->    %d\n", guest_compress_dt);
		if(guest_compress_dt) {
			guest_load_r = dlist->put_off*4096*1000/guest_compress_dt;
		}
	}

	if(dt > 0) {
	    kvm->load_mem_rate = kvm->f_count*4096/dt;
	    //kvm->load_mem_rate = dlist->put_off/2*4096/dt;
    }
//	printk("real load_mem_rate = %d\n", kvm->load_mem_rate);
...
//	kvm->current_runtime = epoch_run_time;
	...
	now = ktime_get();
	diff = ktime_sub(now, vcpu->mark_start_time);
    int epoch_run_time_a = ktime_to_us(diff);
	//int dirty_rate = current_dirty_byte/epoch_run_time_a;
	//current_dirty_byte = dirty_rate*kvm->last_knnTime;
	...
	tmp1 = load_mem_bytes / current_load_mem_rate;
        ...
        kvm->x0[ctx->cur_index] = tmp0;
        kvm->x1[ctx->cur_index] = tmp1;


	int w0 = kvm->w0[ctx->cur_index];
	int w1 = kvm->w1[ctx->cur_index];
...
//	int L3cache = get_l3s(kvm);
	...
	int L3cache = kvm->current_cache_speed;


	int count = kvm->load_mem_rate_rec_index[ctx->cur_index];


//	int L3cache = kvm->L3cache_speed_max;
//	int L3cache = kvm->L3cache_speed;
	int oldcache = L3cache;
...
//	int current_trans_rate = kvm->current_trans_rate;
	...
	//int current_trans_rate = kvm->current_trans_rate[ctx->cur_index];
	int current_trans_rate = kvm->current_trans_rate[0];

	int pr = get_predict_trans_rate_s(kvm, dlist->put_off, current_dirty_byte/4096);
	if(pr > 0)
...
//
        //kvm->x0[ctx->cur_index] = tmp0+tmp1;
//        kvm->x1[ctx->cur_index] = tmp1;

//	   if(current_dirty_byte == 0 || current_trans_rate ==0)
	   if(dlist->put_off == 0 || current_trans_rate ==0)
	   		beta = kvm->w3[ctx->cur_index];
	   else {
       		//beta = kvm->x0*kvm->w0 + kvm->w3 + (current_dirty_byte*1000/current_trans_rate);
		   //beta = (current_dirty_byte*1000/current_trans_rate);
       		//beta = tmp0*w0 + tmp1*w1+ current_dirty_byte*1000/1020;
//       		beta = tmp0*w0 + tmp1*w1;
			//beta = kvm->x0[ctx->cur_index]*w0;
		   //beta = (dlist->put_off*4096*1000/current_trans_rate);
		   beta = (dlist->put_off*4096/current_trans_rate)*1000;
	...
	//	   beta = kvm->w3;
	 //  else
	       //beta = (current_dirty_byte*1000/kvm->current_trans_rate);
	  //     beta = (current_dirty_byte*1000/current_trans_rate);
//       beta = kvm->x0+kvm->x0*kvm->w0+kvm->x1+kvm->x1*kvm->w1 + kvm->w3;
//       beta = kvm->x0*kvm->w0 + kvm->x1*kvm->w1 + L3cache/1000*kvm->w4 + kvm->w3;
/*	if(current_dirty_byte > 4000000) {
		beta = get_predict_trans(kvm, current_dirty_byte, 0);
		beta = beta*1000;
	}
*/
/*	if(beta2 < 0) {
		printk("%ld %d, beta2 = %ld, w3 = %d\n", current_dirty_byte, kvm->current_trans_rate, beta2, kvm->w3);
		if(kvm->current_trans_rate)
		printk(" @ %ld\n", current_dirty_byte/kvm->current_trans_rate);
		printk(" @ %ld\n", current_dirty_byte*1000/kvm->current_trans_rate);
	}
*/
	int time_start = time_in_us();
/*	int predict_transErr = get_predict_transErr(kvm, beta2/1000, L3cache);

	int fix = predict_transErr;
	...
	if(kvm->latency_total <= 50000) {
		...
		if(7<=predict_transErr && 9 >= predict_transErr) {
			fix = 0;
		}
	}
	fix = 0;
	...
	if(predict_transErr == 9) {
		adj = 750;
	} else if (predict_transErr == 10) {
		adj = 1250;
	} else if (predict_transErr == 11) {
		adj = 1750;
	} else if (predict_transErr == 12) {
		adj = 2250;
	} else if (predict_transErr == 13) {
		adj = 2750;
	} else if (predict_transErr == 14) {
		adj = 3250;
	} else if (predict_transErr == 15) {
		adj = 3750;
	}
*/
	int new_beta = beta;
//	int predict_transErr = 100;
	...
	kvm->last_knnTime = time_end-time_start;
//	printk("==================time = %d\n", time_end-time_start);
	now = ktime_get();
	diff = ktime_sub(now, vcpu->mark_start_time);
    //int epoch_run_time = ktime_to_us(diff);
...
/*
	   ...
	   int residual_time = 0;
	   if(!kvm->trans_stop) {
			int trans = kvm->e_trans_latency[(ctx->cur_index+1)%2];
			residual_time = trans-(time_in_us()-kvm->trans_start_r[(ctx->cur_index+1)%2]);
	   }
	   int overlap = 0;
	   if(residual_time > 0) {
		overlap = beta/1000-residual_time;
	   } else {
		overlap = beta/1000;
	   }

	residual_time = overlap - epoch_run_time;
	int nextExtra = 0;
	if(residual_time > 0 && residual_time < beta/1000) {
		nextExtra = residual_time;
	}

//	beta+=nextExtra;
//	extra+=nextExtra;
*/
	int rc = kvm->record_count[ctx->cur_index];
	kvm->dirty_byte_r[ctx->cur_index][rc] = current_dirty_byte;
	kvm->runtime_r[ctx->cur_index][rc] = epoch_run_time;
	kvm->predic_trans[ctx->cur_index][rc] = beta/1000;
	kvm->record_count[ctx->cur_index]++;
	...
	int stall = kvm->last_esti_trans_time - (time_in_us() - kvm->last_trans_time_start);
//	if(stall > 0 && kvm->trans_stop == 0) {
		//beta+=stall;
//	}


//	if(kvm->latency_total > 50000 && !(7<=predict_transErr && 9 >=predict_transErr) /*predict_transErr >= 10*/ )
//	if(kvm->latency_total > 50000 && predict_transErr >= 10)
//	if(kvm->latency_total > 50000 && predict_transErr >= 13)

//	if(kvm->latency_total > 50000 && !(7<=predict_transErr && 9 >=predict_transErr))
//	if(kvm->latency_hit  && /*!(7<=predict_transErr && 9 >=predict_transErr)*/ predict_transErr >= 9 )
//		beta+=fix; //cocotion
...
//	beta+=fix;
	...
	if(kvm->ft_id == 0) {
		...
		//if(ft_m_trans.callsnapshot || epoch_run_time >= target_latency_us-1000)
	//	if(current_dirty_byte > 100000)
	//		isSnapshot = 1;
	//}


//	if(epoch_run_time >= target_latency_us-1000 || beta  >= target_latency_us ) { //this one is good
	if(isSnapshot ) { //this one is good
//	if(epoch_run_time >= target_latency_us-1000 || beta>= target_latency_us-kvm->w2) {
	//if(stall > 0)
		//printk("vmid = %d, beta: %d stall: %d\n", kvm->ft_id, beta, stall);
//	if(kvm->ft_id == 0) {
//		printk("real take snapshot in %d\n", epoch_run_time);
//	}
//		if(predict_transErr < 5) goto notaksnapshot;

//	printk("runtime = %d, trans = %d, total = %d, dirty bytes = %d, w3 = %d\n", epoch_run_time, beta2/1000, epoch_run_time+beta2/1000, current_dirty_byte, kvm->w3);
	kvm->e_round = 1;

/*
	if(epoch_run_time < kvm->last_esti_trans_time) {
		if (kvm->last_esti_trans_time-epoch_run_time-100 > 0)
			usleep_range(kvm->last_esti_trans_time-epoch_run_time-100, kvm->last_esti_trans_time-epoch_run_time);
	}
*/
...
//			last_factor = factor;
        	...
        	kvm->x00[ctx->cur_index] = kvm->x0[ctx->cur_index];
        	kvm->x01[ctx->cur_index] = kvm->x1[ctx->cur_index];
			kvm->w2[ctx->cur_index] = current_dirty_byte;
			kvm->w5[ctx->cur_index] = dlist->put_off*4096;
			kvm->w6[ctx->cur_index] = guest_compress_dt;
			kvm->x6[ctx->cur_index] = guest_load_r;
			kvm->e_latency = beta;
			kvm->e_trans_latency[ctx->cur_index] = beta2;
//			kvm->e_trans_latency_p[ctx->cur_index] = beta3;
			kvm->load_mem_bytes = load_mem_bytes;
			kvm->e_load_mem_rate = kvm->load_mem_rate;
			kvm->e_current_send_rate[ctx->cur_index] = current_send_rate;
			kvm->e_epoch_runtime[ctx->cur_index] = epoch_run_time;
			//kvm->oldcache[ctx->cur_index] = oldcache;
			kvm->oldcache[ctx->cur_index] = current_trans_rate;
			kvm->last_esti_trans_time = beta2/1000;
...
//			kvm->x02[ctx->cur_index] = refactor;
			kvm->x02[ctx->cur_index] = L3cache;
			//kvm->x03[ctx->cur_index] = x3;
			kvm->x03[ctx->cur_index] = extra;
//			kvm->f0[ctx->cur_index] = factor;
			kvm->f0[ctx->cur_index] = last_start;
//			kvm->x02[ctx->cur_index] = 100;
			//kvm->x03[ctx->cur_index] = 100;
			//kvm->f0[ctx->cur_index] = predict_transErr;
		...
		kvm->trans_start_kernel[ctx->cur_index] = time_in_us();
...
//			printk("diff dirty: %d\n", refactor);
//		}
		//printk("--->   kernel trans start (current run end) %d, %d\n", ctx->cur_index, kvm->trans_start_kernel[ctx->cur_index]);
		if(kvm->ft_id == 0)
			ft_m_trans.callsnapshot = 0;
...
int kvm_shm_enable(struct kvm *kvm)
{
	...
	kvm->record_count[kvm->ft_id] = 0;


	kvm->last_esti_trans_time = 0;
	kvm->isflush_while_trans = 0;
	ft_m_trans.callsnapshot = 0;
	kvm->total_cache_ok_sum = 0;
	kvm->total_cache_exceed_sum = 0;
	kvm->total_cache_less_sum = 0;
	kvm->subcount = 0;
	kvm->subhit = 0;
	kvm->phit = 0;
	kvm->phit_count = 0;
	...
	kvm->current_trans_rate[0] = 1000;
	kvm->current_trans_rate[1] = 1000;
	kvm->big_current_trans_rate = 1000;

	ft_m_trans.compress_copy_c = 0;
	ft_m_trans.flush_c = 0;
...
*/
	...
	kvm->trans_rec_index = 0;
	kvm->snapshot_abrupt_c = 0;
	...
	kvm->total_miss = 0;
	kvm->total_miss_time_rec = 0;
	kvm->part_miss_time_rec = 0;
	kvm->sum_diff_right = 0;
	kvm->sum_diff_wrong = 0;
	kvm->count_diff_right = 0;
	kvm->count_diff_wrong = 0;

	kvm->last_knnTime = 100;

	kvm->flush_get_data_ok = 1;
	...
	kvm->mflag = 0;
	kvm->l3s_start = 1;
	kvm->timestamp_index[0] = 0;
	kvm->timestamp_index[1] = 0;
...
//	kvm->latency_hit = -1;
	...
	kvm->L3cache_speed_rec = kmalloc(sizeof(int)*KNUM2, GFP_KERNEL|__GFP_ZERO);
	kvm->L3cache_speed_c = 0;

	kvm->L3cache_speed[0] = 0;
	kvm->L3cache_speed[1] = 0;

	kvm->last_esti_trans_time = 0;
	kvm->last_trans_time_start = time_in_us();


	int i;
	for(i = 0; i < KNUM2; i++)
		ft_m_trans.L3cache_speed_rec[i] = 0;
	ft_m_trans.L3cache_speed_c = 0;

	kvm->L3cache_speed_max = 0;
...
//	kvm->ft_id = atomic_read(&ft_m_trans.ft_vm_count);
	...
	kvm->kdis = kmalloc(sizeof(struct k_dis)*KNUM4, GFP_KERNEL|__GFP_ZERO);
	kvm->kdis3 = kmalloc(sizeof(struct k_dis3)*KNUM4, GFP_KERNEL|__GFP_ZERO);
	kvm->kindex = 0;
	kvm->kpoint = kmalloc(sizeof(struct k_point)*KNUM, GFP_KERNEL|__GFP_ZERO);
	kvm->kpoint2 = kmalloc(sizeof(struct k_point2)*KNUM4, GFP_KERNEL|__GFP_ZERO);
	kvm->kpoint3 = kmalloc(sizeof(struct k_point3)*KNUM4, GFP_KERNEL|__GFP_ZERO);
	kvm->kindex2 = 0;
	kvm->kindex3 = 0;

	kvm->krindex = kmalloc(sizeof(int)*100, GFP_KERNEL|__GFP_ZERO);
	kvm->krindex_ok = kmalloc(sizeof(int)*100, GFP_KERNEL|__GFP_ZERO);

	kvm->krpoint = kmalloc(sizeof(struct k_rpoint*)*100, GFP_KERNEL|__GFP_ZERO);
	for(i = 0; i < 100; i++) {
...
/*
	...
	for(i = 0; i < 10; i++) {
		kvm->kdis[i].index = i;
		kvm->kdis[i].value = 10-i;
	}
	if(kvm->ft_id == 0) {
		printk("before sort\n");
		for(i = 0; i < 10; i++) {
			printk("(%d, %d) ", kvm->kdis[i].index, kvm->kdis[i].value);
		}
		sort(kvm->kdis, 10, sizeof(struct k_dis), &compare_dis, NULL);
		printk("\nafter sort\n");
		for(i = 0; i < 10; i++) {
			printk("(%d, %d) ", kvm->kdis[i].index, kvm->kdis[i].value);
		}
    ...
    struct page *page2, uint8_t *buf)
{
    ...
    for (i = 0; i < 4096; i += 32) {
        ...
        if (memcmp_avx_32(backup + i, page + i)) {
			...
			int index = ft_m_trans.compress_copy_c;
			ft_m_trans.compress_copy[index]++;
			ft_m_trans.compress_copy_c = (ft_m_trans.compress_copy_c+1)%2;
        }
    }
...
//	for(;;) {
		//wait_event_interruptible(kvm->calc_event2, kvm->l3s_start == -1 && kvm->ft_kick2 == 0);
//		break;
//	}
//	printk("fcuk1\n");
...
//	}
	...
	for(i = 0; i < vm_counts; i++)	{
			struct kvm *otherkvm = ft_m_trans.kvm[i];
			if(otherkvm->iscompress)
				vcount++;
	}
	kvm->iscompress = 0;

//	printk("fcuk2, l3start = %d, ft_kick2 = %d, updated = %d\n", kvm->l3s_start, kvm->ft_kick2, kvm->updated);

//	wake_up_interruptible(&kvm->calc_event2);
//	for(;;) {
	//	if(kvm->updated != 1)
	//		wait_event_interruptible(kvm->calc_event2, kvm->updated && kvm->ft_kick2 == 0);
//		break;
//	}
	//kvm->updated = 0;
//	printk("fcuk3\n");
//	printk("fcuk3, l3start = %d, ft_kick2 = %d\n", kvm->l3s_start, kvm->ft_kick2);
...
//	kvm->L3cache_speed_c = (kvm->L3cache_speed_c+1)%KNUM2; //200 is better
//	if(kvm->ft_id == 0)
...
/*	int max = 0;
	for(i = 0; i < KNUM2; i++) {
		if(kvm->L3cache_speed_rec[i] > max) {
			max = kvm->L3cache_speed_rec[i];
		}
	}
*/
...
//	kvm->L3cache_speed_max = max;
	...
	kvm->total_miss_time_rec = kvm->part_miss_time_rec;


	//kvm->part_miss = kvm->cache_end - kvm->cache_start;
...
free:
	...
	if(send_t > 0) {
		...
		//if(kvm->ft_id == 1)
			ft_m_trans.current_send_rate = current_send_rate;
                              ...
                              int max_conn)
{
	...
	kvm->last_trans_time_start = time_in_us();
	//kvm->trans_stall[kvm->trans_rec_index] = time_in_us() - kvm->trans_start_kernel[kvm->trans_rec_index];

	//kvm->trans_start_time = time_in_us();
//	kvm->trans_start_r[kvm->trans_index] = time_in_us();
	//kvm->trans_start_r[kvm->trans_rec_index] = time_in_us();
	//kvm->trans_index = (kvm->trans_index+1)%2;
	//spin_lock(&transfer_lock[kvm->ft_id]);
	kvm->trans_start = 1;
	//spin_unlock(&transfer_lock[kvm->ft_id]);
	...
	//kvm->trans_start_r[kvm->trans_index] = time_in_us();
	kvm->trans_start_r[trans_index] = time_in_us();
	//printk("--->   trans_start_r (current trans start) %d, %d\n", trans_index, kvm->trans_start_r[trans_index]);
...
int kvm_shm_init(struct kvm *kvm, struct kvm_shmem_init *info)
{
	...
	spin_lock_init(&transfer_lock[kvm->ft_id]);
	spin_lock_init(&flush_lock[kvm->ft_id]);
...
void kvmft_bd_update_latency(struct kvm *kvm, struct kvmft_update_latency *update)
{
	...
	kvm->trans_stop_r[update->cur_index] = time_in_us();

	int transindex = 0;
	kvm->ft_kick2 = 1;
	int myl3s = kvm->L3cache_speed[transindex];
...
/*	if(pedding > 300 && last_trans_end - current_run_end < 0) {
//	if(current_trans_start-current_run_end < 0) {
//		printk("current_trans_start index = %d %d\n",kvm->trans_rec_index, current_trans_start);
//		printk("current run end index = %d %d\n",update->cur_index, current_run_end);
//		printk("current_trans_start-current_run_end = %d\n", current_trans_start-current_run_end);
		printk("last_trans_end = %ld\n", last_trans_end);
		printk("current_run_end = %ld\n", current_run_end);
		printk("last_trans_end - current_run_end = %d %d\n", update->cur_index, last_trans_end-current_run_end);
		printk("current_trans_start-current_run_end = %d\n", current_trans_start-current_run_end);
		kvm->snapshot_abrupt_c++;
	} else if (pedding > 300 && last_trans_end - current_run_end > 0) {
		kvm->trans_too_long_c++;
		printk("@@@@@@@@@@@@#### lasttrans = %d, trans = %d\n", kvm->last_trans, update->trans_us);
	}
*/
//	}
//
	kvm->last_trans = update->trans_us;
//	printk("curindex = %d, current_run_end = %ld, last_trans_end = %ld, current_trans_start = %ld, pending = %ld, realpending = %ld\n", \
			ctx->cur_index, \
			current_run_end, last_trans_end, current_trans_start, \
			current_trans_start-current_run_end, \
			last_trans_end-current_run_end);
//	printk("last trans time = %ld\n", last_trans_end - current_trans_start_old);

//	printk("%d\n", time_in_us()-kvm->last_trans_end);

	kvm->last_trans_end[update->cur_index] = time_in_us();
	//printk("ok trans end = %d %ld\n", update->cur_index, kvm->last_trans_end[update->cur_index]);
	//kvm->trans_stop_time = time_in_us();
	s64 trans_stop_time = time_in_us();
	//int real_trans = trans_stop_time - kvm->timestamp[kvm->flush_index][0];
//	int real_trans = update->trans_us-kvm->trans_stall[kvm->trans_rec_index];
	int real_trans = time_in_us() - current_trans_start;
...
//	if(real_trans < 0) printk("wtf!!\n");
	...
	s64 thisTrans_stop = kvm->trans_stop_r[update->cur_index];
		...
		int this_dirty_bytes_s = 0;
		int this_trans_rate_s = 1000;
		int this_dirty_bytes_b = 0;
		int this_trans_rate_b = 1000;

	    int dirty_pfns_len = update->average_de;

		int tr = 0;
			if(real_trans > 0) {
				//int tr = update->dirty_page/real_trans;
				tr = dirty_pfns_len*4096/real_trans;
				if(tr)
					//kvm->current_trans_rate[update->cur_index] = tr;
					kvm->current_trans_rate[0] = tr;
			}

/*		if(real_trans > 0 && kvm->x03[update->cur_index]==-999) {
			int tr = update->dirty_page/real_trans;
			if(tr) {
...
//	printk("vmid = %d, cocotion: orig = %d, stall = %d, real = %d, e_runtime = %d, runtime = %d\n", kvm->ft_id, update->trans_us, kvm->trans_stall[ctx->cur_index], real_trans, update->e_runtime, update->runtime_us);
	...
	//printk("orig trans: %d, real trans: %d\n", update->trans_us, real_trans);


	kvm->latency_total++;
/*	if(kvm->latency_total % 1000 == 0) {
		printk("snapshot: %ld, %ld, %d\n", kvm->snapshot_abrupt_c, kvm->latency_total, kvm->snapshot_abrupt_c/kvm->latency_total);
		printk("realpending: %ld, %ld, %d\n", kvm->trans_too_long_c, kvm->latency_total, kvm->trans_too_long_c/kvm->latency_total);
	}*/
	long long real_f = 0;
	kvm->real_f = real_f;
	...
	update->e_dirty_bytes = kvm->w2[cur_index];
	update->e_latency = kvm->e_latency;

	update->e_runtime = kvm->e_epoch_runtime[cur_index];
	update->e_trans   = kvm->e_trans_latency[cur_index]/1000;

	int e_trans_us = kvm->e_trans_latency[cur_index];
	e_trans_us /= 1000;
...
//	update->f_trans = (kvm->w0*fix_trans_x0 + kvm->w1*fix_trans_x1+kvm->w3)/1000;
	...
	int current_dirty_byte = kvm->w2[cur_index];
...
//	int trans_diff = update->trans_us - e_trans_us;
	int trans_diff = real_trans - e_trans_us;

	update->last_load_mem_rate = -1;

//	if(trans_diff > 500 || trans_diff < -500) {
/*
	int sum = 0;
	for(k = 0; k < count2; k++) {
		//if(kvm->load_mem_rate_rec[kvm->flush_index][k] > sum)
		//	sum = kvm->load_mem_rate_rec[kvm->flush_index][k];
		sum += kvm->load_mem_rate_rec[kvm->flush_index][k];
	}
	if(count2 > 0) {
		update->last_load_mem_rate = sum/count2;
		//update->last_load_mem_rate = sum;
		update->load_mem_rate = trans_diff*100/e_trans_us;
	}*/
//	}
//
/*	s64 real_trans_start = kvm->timestamp[kvm->flush_index][0] ;
	s64 real_trans_end = real_trans_start+update->trans_us;

	int ii;
	int timestamp_index = kvm->timestamp_index[kvm->flush_index];
	for(ii = 0; ii < timestamp_index; ii++) {
		if(real_trans_end < kvm->timestamp[kvm->flush_index][ii]) {
			if(ii > 0) ii-=1;
			break;
		}
	}
	if(ii == timestamp_index) ii = timestamp_index-1;
	int start_index = 0;
	int end_index = ii;

	s64 time_start = real_trans_start;
	s64 time_end = kvm->timestamp[kvm->flush_index][end_index];

	uint64_t cache_start = kvm->load_mem_rate_rec[kvm->flush_index][0];
...
//		printk("diff = %d bias = %d\n", kvm->latency_diff, kvm->latency_bias);
		kvm->lc = 0;
	//	miss = 0;
	//	hit = 0;
		kvm->learningR = LEARNRATE;
...
//	if(e_trans_us >= update->trans_us && update->e_runtime < update->runtime_us && latency_us > 1100) {
//		kvm->latency_total3++;
//	} else {
//		kvm->latency_total4++;
//	}
//	printk("%d\n", update->runtime_us - update->e_runtime);
/*	if(update->runtime_us < update->e_runtime) {
		kvm->flush_index = (kvm->flush_index+1)%2;
		//return ;
	}
	else {
		//update->trans_us += (update->runtime_us - update->e_runtime);
		real_trans += (update->runtime_us - update->e_runtime);
	}
*/
...
/*
	...
	s64 real_trans_start = kvm->timestamp[kvm->flush_index][0] ;
	//s64 real_trans_end = real_trans_start+update->trans_us;
	s64 real_trans_end = trans_stop_time;
	...
	for(ii = 0; ii < end_index; ii++) {
		int start = ii;
		int end = ii + 1;
		if(end >= end_index) break;
		s64 time1 = kvm->timestamp[kvm->flush_index][start];
		s64 time2 = kvm->timestamp[kvm->flush_index][end];
		uint64_t cache1	 = kvm->load_mem_rate_rec[kvm->flush_index][start];
		uint64_t cache2	 = kvm->load_mem_rate_rec[kvm->flush_index][end];

		int delta = time2-time1;
		if(delta > 0) {
			int speed = (long long)(cache2-cache1)*1000/delta;
			//printk("%d %d %d\n", delta, cache2-cache1, speed);
			if(speed > mis_max ) {
				mis_max = speed;
			}
		}
...
//	printk("average: %d %d %d\n", (long long)(cache_end-cache_start), time_end-time_start, (long long)(cache_end-cache_start)*1000/(time_end-time_start));
//	printk("%d %lld %lld %lld %lld\n", kvm->flush_index, cache_end, cache_start, time_end, time_start);
...
//	trans_diff = update->trans_us - e_trans_us;
	trans_diff = real_trans - e_trans_us;
...
//	printk("e_trans = %d\n", e_trans_us);
		...
		int kr = dirty_pfns_len/100;
		if(kr >= 100) kr = 99;
		if(dirty_pfns_len) {
			kvm->krpoint[kr][kvm->krindex[kr]].dirty_pfns_len = dirty_pfns_len;
			kvm->krpoint[kr][kvm->krindex[kr]].dirty_len = update->dirty_page/4096;
			kvm->krpoint[kr][kvm->krindex[kr]].trans_rate = tr;
			if(kvm->krindex[kr]+1 == KNUM4)
				kvm->krindex_ok[kr] = 1;
			kvm->krindex[kr] = (kvm->krindex[kr]+1)%KNUM4;
		}
/*		if(update->dirty_page > 0 && this_dirty_bytes_s > 0) {
//			kvm->kpoint3[kvm->kindex3].dirty_byte = update->dirty_page;
//			kvm->kpoint3[kvm->kindex3].trans_time = kvm->current_trans_rate;
			kvm->kpoint3[kvm->kindex3].dirty_byte = this_dirty_bytes_s;
			kvm->kpoint3[kvm->kindex3].trans_time = this_trans_rate_s;
			kvm->kindex3 = (kvm->kindex3+1)%KNUM4;
		}
		if(update->dirty_page > 0 && this_dirty_bytes_b > 0) {
			kvm->kpoint2[kvm->kindex2].current_dirty_byte = this_dirty_bytes_b;
			kvm->kpoint2[kvm->kindex2].current_L3cache_speed = this_trans_rate_b;
			kvm->kindex2 = (kvm->kindex2+1)%KNUM4;
		}
*/
...
/*		for(i = 0; i < KNUM; i++) {
			if(e_trans_us == kvm->kpoint[i].estimated_transtime && e_trans_us \
			 && kvm->kpoint[i].L3cache_speed == L3cache  ) {
				kvm->kpoint[i].transtime_err = trans_diff*100/(int)e_trans_us;
				i = -1;
				break;
			}
		}
		...
		if(i!=-1 && e_trans_us) {
			...
			kvm->kpoint[kvm->kindex].transtime_err = trans_diff*100/(int)e_trans_us;
...
//
//
/*	if(update->dirty_page > 4000000) {
		kvm->kpoint3[kvm->kindex3].dirty_byte = kvm->w2[cur_index];
		kvm->kpoint3[kvm->kindex3].trans_time = update->trans_us;
		kvm->kindex3 = (kvm->kindex3+1)%KNUM4;
	}
*/
/*	if(kvm->subcount == 1000) {
		printk("@@@@@@@@@@@@@@: subcount = %d, subhit = %d, %d\n", kvm->subcount, kvm->subhit, kvm->subhit*100/kvm->subcount);
		kvm->subcount = 0;
		kvm->subhit = 0;
	}*/
	kvm->subcount++;
	if(latency_us <= target_latency_us + 1000 && latency_us >= target_latency_us -1000) {
		if(kvm->lasthit) {
			...
			kvm->qmiss_count++;
			//printk("(%d, %d) 1miss, Qmiss = %d\n",  kvm->qmiss, kvm->qmiss_count, kvm->qmiss/kvm->qmiss_count);
	;	}
		kvm->subhit++;
		kvm->lasthit = 1;
	} else {
		if(kvm->lasthit) {
			kvm->phit_count++;
			//printk("(%d, %d) Phit, 1miss = %d\n", kvm->phit, kvm->phit_count, kvm->phit/kvm->phit_count);
		} else {
			kvm->qmiss++;
		}
		kvm->lasthit = 0;
	...
	int ori = diff_latency(trans_diff);
	int el = kvm->f0[ctx->cur_index];

	int dd = real_L3cache-L3cache;
	int dda = 0;
...
//	if(kvm->latency_total > KNUM) {
	if(/*trans_diff > 0 && */(kvm->latency_total > KNUM && kvm->kindex == KNUM-1)||kvm->latency_total2) {
		kvm->latency_total2++;
		kvm->sum_actual_miss_speed+=real_L3cache;
		if((ori - el <=1 && ori - el >= 0) || (el - ori <= 1 && el - ori >= 0 )) {
		 	kvm->total_1c++;
/*
			printk("============================1start=============\n");
			printk("too large (%d %d %d)\n", kvm->latency_total3, kvm->latency_total4, kvm->latency_total);
			printk("cache speed = %d\n", ft_m_trans.L3cache_speed);
			printk("latency bias %d, lc = %d\n", kvm->latency_bias, kvm->lc);
		    printk("%d (1ms error) %ld %ld %ld, %d %d\n", kvm->ft_id, kvm->total_1c, kvm->latency_total2, kvm->total_1c*100/kvm->latency_total2, ori, el);
			printk("w0: %d, w1: %d w3: %d w4: %d\n", kvm->w0, kvm->w1, kvm->w3, kvm->w4);
*/
            ...
            //printk("============================1end=============\n");
			//
	/*		int mydiff = (real_L3cache-L3cache);
			if(mydiff < 0) mydiff*=-1;
			kvm->sum_diff_right += mydiff;
			kvm->count_diff_right++;*/
	//		printk("%d %d %d\n", real_L3cache, L3cache, mydiff);
	//		printk("vid = %d: L3 right average = %ld\n", kvm->ft_id, kvm->sum_diff_right/kvm->count_diff_right);
	//		printk("vid = %d: L3 real average = %ld\n", kvm->ft_id, kvm->sum_actual_miss_speed/kvm->latency_total2);

		} /*else {
			int mydiff = (real_L3cache-L3cache);
			if(mydiff < 0) mydiff*=-1;
			kvm->sum_diff_wrong += mydiff;
			kvm->count_diff_wrong++;
		//	printk("vid = %d: L3 wrong average = %ld\n", kvm->ft_id, kvm->sum_diff_wrong/kvm->count_diff_wrong);
		}*/


        if((ori - el <=2 && ori - el >= 0) || (el - ori <= 2 && el - ori >= 0 )) {
		    kvm->total_1_5c++;
//		    printk("%d (1.5ms error) %ld %ld %ld, %d %d\n", kvm->ft_id, kvm->total_1_5c, kvm->latency_total2, kvm->total_1_5c*100/kvm->latency_total2, ori, el);
				/*
			printk("============================1.5start=============\n");
        ...
        if((ori - el <=3 && ori - el >= 0) || (el - ori <= 3 && el - ori >= 0 )) {
		    kvm->total_2c++;
//		    printk("%d (2ms error) %ld %ld %ld, %d %d\n", kvm->ft_id, kvm->total_2c, kvm->latency_total2, kvm->total_2c*100/kvm->latency_total2, ori, el);
				/*
			printk("============================2start=============\n");
...
////verify end
	...
	int predCache = kvm->current_cache_speed;

//	printk("%lld %lld %lld %lld\n", cache_end, cache_start, time_end, time_start);

/*	if(cache_end-cache_start>=0 && time_end-time_start>0 && time_end-time_start2 > 0 && e_trans_us) {
	update->last_load_mem_rate = (long long)(cache_end-cache_start)*1000/(time_end-time_start);
	kvm->current_cache_speed = update->last_load_mem_rate;
//	update->last_load_mem_rate = mis_max;
	update->load_mem_rate = trans_diff*100/e_trans_us;
...
//	kvm->cache_hist[kvm->flush_index] = (cache_end-cache_start2)*1000/(time_end-time_start2);
	//update->w5 = kvm->current_cache_speed;
	//update->w5 = kvm->oldcache[cur_index];
	update->w5 = update->trans_us;
	update->last_load_mem_rate = real_trans;
	update->load_mem_rate = kvm->w2[cur_index];
...
//	}
	...
	update->alpha =  kvm->e_epoch_runtime[cur_index]+kvm->e_trans_latency[cur_index]/1000;

	int rc = kvm->record_count[cur_index];
	kvm->record_count[cur_index] = 0;

	int fc = ft_m_trans.flush_c;



	if(kvm->latency_hit == 0 && fix == 0/*&& kvm->latency_total < 50000*/) {
	if(latency_us <= target_latency_us + 1000 && latency_us >= target_latency_us -1000) {
		kvm->total_cache_ok_sum+=kvm->current_cache_speed;
		...
		if(kvm->ft_id == 0) {
			printk("------okokok-------------------------@@@ start\n");
			printk("predic tranbyte = %d, real transbyte = %d\n", kvm->w2[cur_index], update->dirty_page);
			if(update->dirty_page)
				printk("uncompress tranbyte = %d, real transbyte = %d, compress_rate = %d\n", kvm->w5[cur_index], update->dirty_page, kvm->w5[cur_index]*100/update->dirty_page);
			//printk("predic transrate = %d, real transrate = %d\n", kvm->oldcache[cur_index], update->dirty_page/real_trans);
			printk("uncompress tranbyte = %d, real uncompress = %d\n", kvm->w5[cur_index], dirty_pfns_len*4096);
			printk("predic transrate = %d, real transrate = %d\n", kvm->oldcache[cur_index], dirty_pfns_len*4096/real_trans);
			printk("predic runtime = %d, real runtime = %d\n", kvm->e_epoch_runtime[cur_index], update->runtime_us);
			printk("predic trans = %d, real trans = %d\n", kvm->e_trans_latency[cur_index]/1000, update->trans_us);
			printk("predic latency = %d, real latency = %d ", kvm->e_epoch_runtime[cur_index]+kvm->e_trans_latency[cur_index]/1000, update->latency_us);
			printk("total copy in compress = %d\n", ft_m_trans.compress_copy[fc]);
			printk("no pendding latency = %d \n", update->runtime_us+real_trans);
			printk("predic chache = %d, real cache = %d\n", predCache, kvm->current_cache_speed);
			printk("update->trans = %d, real trans = %d\n", update->trans_us, real_trans);
			printk("x0 = %d, w0 = %d, w3 = %d\n", kvm->x0[cur_index], kvm->w0[cur_index], kvm->w3[cur_index]);
			printk("current_send_rate = %d, real_send_rate = %d\n", kvm->e_current_send_rate[cur_index], ft_m_trans.current_send_rate);
			printk("okCache = %ld %ld %ld\n", kvm->total_cache_ok_sum, kvm->latency_ok, kvm->total_cache_ok_sum/kvm->latency_ok);
			printk("index = %d, last trans end = %ld\n", cur_index, last_trans_end);
			printk("current run end = %ld, current trans start = %ld\n", current_run_end, current_trans_start);
			printk("current run start = %ld, in kernel runtime = %ld\n", current_run_start, current_run_end-current_run_start);

			printk("extra = %d\n", extra);
			printk("gdt = %d\n", kvm->w6[cur_index]/1000);
			printk("gload_r = %d\n", kvm->x6[cur_index]/1000);
			printk("-------------------------------@@@ end\n");
		}
		return;
	} else if (latency_us > target_latency_us + 1000) {
		kvm->total_cache_exceed_sum+=kvm->current_cache_speed;
		kvm->exceed_c++;

		if(kvm->ft_id == 0) {
			printk("------too long-------------------------@@@ start\n");
			printk("predic tranbyte = %d, real transbyte = %d\n", kvm->w2[cur_index], update->dirty_page);
			if(update->dirty_page)
				printk("uncompress tranbyte = %d, real transbyte = %d, compress_rate = %d\n", kvm->w5[cur_index], update->dirty_page, kvm->w5[cur_index]*100/update->dirty_page);
			//printk("predic transrate = %d, real transrate = %d\n", kvm->oldcache[cur_index], update->dirty_page/update->trans_us);
			printk("uncompress tranbyte = %d, real uncompress = %d\n", kvm->w5[cur_index], dirty_pfns_len*4096);
			printk("predic transrate = %d, real transrate = %d\n", kvm->oldcache[cur_index], dirty_pfns_len*4096/real_trans);
			printk("predic runtime = %d, real runtime = %d\n", kvm->e_epoch_runtime[cur_index], update->runtime_us);
			printk("predic trans = %d, real trans = %d\n", kvm->e_trans_latency[cur_index]/1000, update->trans_us);
			...
			printk("x0 = %d, w0 = %d, w3 = %d\n", kvm->x0[cur_index], kvm->w0[cur_index], kvm->w3[cur_index]);
			printk("current_send_rate = %d, real_send_rate = %d\n", kvm->e_current_send_rate[cur_index], ft_m_trans.current_send_rate);
			printk("exceedCache = %ld %ld %ld\n", kvm->total_cache_exceed_sum, kvm->exceed_c, kvm->total_cache_exceed_sum/kvm->exceed_c);
			printk("index = %d, last trans end = %ld\n", cur_index, last_trans_end);
			printk("current run end = %ld, current trans start = %ld\n", current_run_end, current_trans_start);
			printk("current run start = %ld, in kernel runtime = %ld\n", current_run_start, current_run_end-current_run_start);
			printk("extra = %d\n", extra);
			...
			int i;
/*			for(i = 0; i < rc; i++) {
				printk("%d, runtime = %d\n", i, kvm->runtime_r[cur_index][i]);
				printk("%d, dirtybyte = %d\n", i, kvm->dirty_byte_r[cur_index][i]);
				printk("%d, predic_trans = %d\n", i, kvm->predic_trans[cur_index][i]);
			}
*/
		...
		kvm->total_cache_less_sum+=kvm->current_cache_speed;
		kvm->less_c++;
		if(kvm->ft_id == 0) {
			printk("------too short-------------------------@@@ start\n");
			printk("predic tranbyte = %d, real transbyte = %d\n", kvm->w2[cur_index], update->dirty_page);
			if(update->dirty_page)
				printk("uncompress tranbyte = %d, real transbyte = %d, compress_rate = %d\n", kvm->w5[cur_index], update->dirty_page, kvm->w5[cur_index]*100/update->dirty_page);
			printk("uncompress tranbyte = %d, real uncompress = %d\n", kvm->w5[cur_index], dirty_pfns_len*4096);
			...
			printk("x0 = %d, w0 = %d, w3 = %d\n", kvm->x0[cur_index], kvm->w0[cur_index], kvm->w3[cur_index]);
			printk("current_send_rate = %d, real_send_rate = %d\n", kvm->e_current_send_rate[cur_index], ft_m_trans.current_send_rate);
			printk("lessCache = %ld %ld %ld\n", kvm->total_cache_less_sum, kvm->less_c, kvm->total_cache_less_sum/kvm->less_c);
			printk("index = %d, last trans end = %ld\n", cur_index, last_trans_end);
			printk("current run end = %ld, current trans start = %ld\n", current_run_end, current_trans_start);
			printk("current run start = %ld, in kernel runtime = %ld\n", current_run_start, current_run_end-current_run_start);
			printk("extra = %d\n", extra);
	...
	ft_m_trans.compress_copy[fc] = 0;
	ft_m_trans.flush_c = (ft_m_trans.flush_c+1)%2;

	//	int less_f = kvm->less_c*100/kvm->latency_total;
//	int exceed_f = kvm->exceed_c*100/kvm->latency_total;
	int less_f = kvm->less_c*100/(kvm->latency_ok+kvm->less_c+kvm->exceed_c);
	int exceed_f = kvm->exceed_c*100/(kvm->latency_ok+kvm->less_c+kvm->exceed_c);

	int adj = (exceed_f-less_f)*100;

	if(kvm->latency_hit == 0 && fix == 0)
	if(!(adj <= 100 && adj >= -100) /*&& kvm->latency_total < 50000*/) {
		int w3 = kvm->w3;
		w3 += adj;
		if (w3 < 5000)
...
//	if(update->dirty_page==0  || (kvm->w0 <= 1000 && kvm->w1 <= 1000) ) {
	//if(update->dirty_page==0  || kvm->w0 <= 1000 ) {
//	if(update->dirty_page==0 || kvm->x00[cur_index] == 0 || kvm->x01[cur_index] == 0 /*|| (kvm->w0 <= 1000 && kvm->w1 <= 1000)*/ ) {
//	if(update->dirty_page==0 /*&& real_trans > update->e_trans*/) {
		if(kvm->lcc > 1000)  {
			kvm->lcc = 0;
			kvm->average_w3 = 0;
		}
		kvm->lcc++;
		int w3 = kvm->w3[cur_index];
	//	if(update->trans_us-update->e_trans > 0 ) {
		//if(adj <= 100 )
//		w3 = kvm->w3 + (update->trans_us - update->e_trans) * 1000;
//		if(real_trans > update->e_trans)
			w3 = kvm->w3[cur_index] + (real_trans - update->e_trans) * 1000;
	//	kvm->lcc = 0;
	//	}
		if (w3 < 5000)
			w3 = 5000;
		...
		kvm->average_w3 += w3;
		w3 = kvm->average_w3/(kvm->lcc);
		//int w3 = update->trans_us * 1000;
		//int w3 = kvm->w3 + (update->trans_us - (kvm->x00[cur_index]*kvm->w0+kvm->x01[cur_index]*kvm->w1+kvm->w3)/1000) * 1000;
		//int w3 = (latency_us-kvm->e_latency)*1000;
		//int w3 = kvm->w3 - kvm->latency_bias*1000;
...
//	if(e_trans_us < real_trans-500  && kvm->latency_hit == 0) {
//	if(latency_us > target_latency_us + 1000  && kvm->latency_hit == 0) {
	if(/*(latency_us > target_latency_us + 1000) &&*/ e_trans_us < real_trans  && kvm->latency_hit == 0 && fix == 0) {
//	if(e_trans_us < update->trans_us-500 && kvm->latency_hit == 0) {
//		kvm->latency_total3++;
        //if(update->dirty_page != 0) {
			kvm->latency_total3++;
			//if(real_f == 0) {
			int w0 = kvm->w0[cur_index];
			int w1 = kvm->w1[cur_index];
			if(ft_m_trans.compress_copy_c%2 == 0)
		    w0 = kvm->w0[cur_index] + (kvm->learningR*kvm->x00[cur_index]*(1))/1000;
			else
		    w1 = kvm->w1[cur_index] + (kvm->learningR*kvm->x01[cur_index]*(1))/1000;
...
//			if(w1 > 4000) w1 = 4000;
			...
			kvm->w4 = kvm->w4 + (kvm->learningR*kvm->x02[cur_index]/1000*(1))/1000;


			//	int w4 = kvm->w4 + (kvm->learningR*kvm->x02[cur_index]*(1))/1000;
		...
		kvm->learningR = (kvm->learningR * 1000) / 1200; //@@
...
//    } else if (e_trans_us > real_trans+500 && kvm->latency_hit == 0) {
    //} else if (latency_us < target_latency_us - 1000 && kvm->latency_hit == 0) {
    } else if (/*(latency_us < target_latency_us - 1000) &&*/ e_trans_us > real_trans && kvm->latency_hit == 0 && fix == 0) {
...
//			kvm->latency_total4++;
        //if(update->dirty_page != 0) {
		    ...
		    w1 = kvm->w1[cur_index] + (kvm->learningR*kvm->x01[cur_index]*(-1))/1000;
			int w4 = kvm->w4 + (kvm->learningR*kvm->x02[cur_index]/1000*(-1))/1000;
//			if(w0 < 1000 ) w0 = 1000;
//			if(w1 < 1000 ) w1 = 1000;
			//if(w0 < 1000+adj ) w0 = 1000+adj;
			//if(w1 < 1000+adj ) w1 = 1000+adj;
//			if(w0 < 1000 ) w0 = 1000;
			if(w0 < 1000 ) w0 = 1000;
			if(w1 < 1000 ) w1 = 1000;
	//		if(w0 < 0 ) w0 = 0;
...
//					ft_m_trans.w4 = w4;
	...
	//if(kvm->latency_hit == 0)
	//if(kvm->x00[cur_index] == 0 || kvm->x01[cur_index] == 0 ) {
	//if(update->dirty_page==0 || kvm->x00[cur_index] == 0 || kvm->x01[cur_index] == 0 || (kvm->w0 <= 1000 && kvm->w1 <= 1000) ) {
/*	if(update->dirty_page==0 ) {
		//int w3 = kvm->w3 + (update->trans_us - update->e_trans) * 1000;
		int w3 = update->trans_us * 1000;
...
*/
	/*
	int w3;
	if(real_trans - (kvm->x00[cur_index]*kvm->w0+kvm->x01[cur_index]*kvm->w1+kvm->w3)/1000 > 0) {
		kvm->w3+=1000;
	} else if(real_trans - (kvm->x00[cur_index]*kvm->w0+kvm->x01[cur_index]*kvm->w1+kvm->w3)/1000 < 0) {
		w3 = kvm->w3 - 1000;
		if (w3 < 0)
			w3 = 0;
		kvm->w3 = w3;
